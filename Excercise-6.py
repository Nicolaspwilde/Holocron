...
print(
    f"My name is {My_information['First_name']} {My_information['Middle_name']} {My_information['Last_name']}"
)
...
my_dict = {"a": 1, "b": 2, "c": 1, "d": 3, "e": 2}  # duplicate value  # duplicate value
print("unique values : ")
...
for river in rivers.keys():
    if river == "Indus":
        ...
        print(
            f"{river} is an important river which flows in these countries that are : {countries} "
        )
    if river == "Nile":
        countries = " , ".join(rivers[river])
        print(
            f"{river} is an important river which flows in middle east in countries like this : {countries}"
        )
