...
customer_input = input(
    f"pls enter your toppings choice these are available toppings{list_of_toppings}"
).split(" ")
...
for toppings in list_by_customer:
    if toppings in list_of_toppings:
        ...
        suggestion = input(
            "Do you want to make suggestion to add this toppings to list y or n ?"
        ).lower()
        if suggestion == "y":
            suggestion_for_list = input("enter the topping name with spaces").split(" ")
            new_toppings = [
                suggested_toppings.strip().lower()
                for suggested_toppings in suggestion_for_list
            ]
